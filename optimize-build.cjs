...
try {
  ...
  // Crear un archivo index.cjs compatible en dist para respaldo
  const indexContent = `// Archivo generado automáticamente para despliegue
process.env.NODE_ENV = 'production';

try {
  // Intentar importar el servidor de producción en formato CommonJS
  require('./production-server.cjs');
} catch (error) {
  console.error('❌ Error al iniciar el servidor en modo CommonJS:', error);
  
  // Intentar un enfoque alternativo con servidor directo
  try {
    require('./server.js');
  } catch (err) {
    console.error('❌ Error crítico al iniciar cualquier servidor:', err);
    process.exit(1);
  }
}
`;
